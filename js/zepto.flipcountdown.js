...
(function ($) {
    ...
    var expando = "Zepto" + ("1" + Math.random()).replace(/\D/g, '');
    $.dataCache = function() {
    ...
    $.fn.dataCache = function (name, value) {
        return $.dataCache(this[0],name,value);
    };
    ...
    $.fn.flipCountDown = $.fn.flipcountdown = function (_options) {
                ...
                var $flipcountdown = $('<div class="xdsoft_flipcountdown"></div>'),
                    ...
                    _generate = function (chars) {
                        ...
                        for (var i = 0, n = chars.length; i < n; i++) {
                            ...
                            if (blocks[i].dataCache('value') != chars[i]) {
                                if (!_typeCompare(blocks[i].dataCache('value'), chars[i])) {
                                    blocks[i]
                                ...
                                if (!isNaN(chars[i])) {
                                    var old = parseInt(blocks[i].dataCache('value')),
                                        ii = parseInt(blocks[i].dataCache('i')),
                                ...
                                blocks[i].dataCache('value', chars[i]);
                                blocks[i].dataCache('i', i);
                            }
                        }
        ...
        return this.each(function () {
            var $box = $(this);
            if (!$box.dataCache('setOptions')) {
                $box.addClass('xdsoft');
                createFlipCountDown($box);
            }
            $box.dataCache('setOptions') &&
            $.isFunction($box.dataCache('setOptions')) &&
            $box.dataCache('setOptions')(_options);
        });
    }
})(Zepto);